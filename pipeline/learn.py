"""GridIron's learning loop for the pregame odds model.

Once a week, after every game of the week is final, it re-tests each weight group in learning/spec.json:
  ...
  2. Compare.   Score those walk-forward values against the weights that were actually in force, game by game, on
                games neither side had seen. Evidence is resampled by week to get a confidence level, corrected for
                how many ideas were tested in the same review.
  ...
  6. Act.       A small move within limits applies automatically (at most one per review, at most one step per
                category per season, a cooldown between changes). A big move, or switching a factor on or off, becomes
                a proposal for the owner. A change that then does worse on new games than the version it replaced is
                rolled back automatically.

  python3 learn.py review [--force]      weekly review (skips when no new completed week)
...
"""
import os, sys, json, math, zlib, argparse, datetime
...
SPEC=gm.load_spec(); SC=SPEC['score']; CATS={c['id']:c for c in SPEC['categories']}
...
def fmt(cat,P):
    ...
    if cat['kind']=='engine_ratio':
        O=None; return ', '.join('%s %g'%(x,P[x]) for x in p)
...
# ------------------------------------------------------------------ a review
MLAB={'margin':'margin error','total':'total error','margin_total':'average of margin and total error','logloss':'win-probability log loss','blend':'blended-line error'}

def evaluate(fr,C,cutoff,review_id,only=None,state=None):
    ...
    for cat in SPEC['categories']:
        ...
        def tuner(V):
            ...
            return tuners[V['v']]
        # what the data supports now, from the current champion
        T=tuner(Cur); i_all=fr.n; best=clip(cat,T.best(i_all)); cur=T.cur
        res=dict(category=cat['id'],name=cat['name'],what=cat['what'],metric=cat['metric'],
        ...
        variants=[]
        full_steps=steps(cat,cur,best); onoff=is_off(cur)!=is_off(best)
        variants.append(('full',best))
        if full_steps>1+1e-9 and not onoff: variants.append(('step',toward(cat,cur,best,1.0)))
        for vname,target in variants:
            ...
            for w in weeks:
                ...
                pb=predict(fr,V['params'],idx)
                pc,_=Tv.pred_at(clip(cat,tv),idx)
    ...
    for c in tested: judge(c,len(tested),cutoff,state or {})
    return cands

def judge(c,m,cutoff,state):
    cat=CATS[c['category']]; S=SPEC; sec=c['sec']; tol=S['collateral']
    big=[b for b in c['blocks'] if b['n']>=S['min_block_games']]; better=sum(1 for b in big if b['cand']<b['base']); worse=len(big)-better
    col=[k for k in ('margin','total','logloss') if k!=c['metric'] and sec[k][1]-sec[k][0]>tol[k]]
    checks=[('enough',c['n']>=S['min_eval_games'],'%d games it never saw (need %d)'%(c['n'],S['min_eval_games'])),
            ('better',c['delta']>0,'%s %.3f → %.3f'%(MLAB[c['metric']],c['base'],c['cand'])),
    ...
    if c['delta']>0: pros.append(('Lower %s on %d games it never saw: %.3f → %.3f'%(MLAB[c['metric']],c['n'],c['base'],c['cand']),min(z,SC['z_cap'])))
    ...
    for k,lab in (('margin','margin error'),('total','total error'),('logloss','win-probability log loss')):
        if k==c['metric']: continue
        dlt=sec[k][0]-sec[k][1]
        if dlt>tol[k]/2: pros.append(('Also improves %s (%.3f → %.3f)'%(lab,sec[k][0],sec[k][1]),SC['secondary_better']))
        elif -dlt>tol[k]/4: cons.append(('Slightly worse %s (%.3f → %.3f)'%(lab,sec[k][0],sec[k][1]),SC['secondary_worse']))
    if sec['ats'][1]-sec['ats'][0]>=1.0: pros.append(('Against the spread %.1f%% → %.1f%%'%sec['ats'],SC['secondary_better']))
    elif sec['ats'][0]-sec['ats'][1]>=1.0: cons.append(('Against the spread %.1f%% → %.1f%%'%sec['ats'],SC['secondary_worse']))
    ...
    if c['metric'] in ('margin','margin_total') and sec['gap'][1]-sec['gap'][0]>0.1:
        cons.append(('Further from the closing line, which has been the more accurate number (%.2f → %.2f pts apart)'%sec['gap'],SC['further_from_market']))
    ...
    if not hard: c['status']='watching' if c['delta']>0 else 'rejected'
    elif c['net']<=0: c['status']='rejected'
    else: c['status']='passes'
    small=(not c['onoff']) and c['steps']<=1+1e-9
    c['size']='small' if small else 'big'
...
def evidence_path(did): return os.path.join(EDIR,'%s.json'%did.replace(':','_'))

def review(force=False):
    book=gm.Book(); cutoff=latest_cutoff(book); state=jload(P_STATE,{}); C=gm.load_champion(); LOG=jload(P_LOG,[]); PROPS=jload(P_PROPS,[])
    if cutoff is None: print('no completed week yet -- nothing to learn from'); return
    if state.get('last_cutoff')==cutoff and not force:
        print('week %d-%02d already reviewed -- nothing new to learn from'%tuple(cutoff)); write_summary(C,LOG,PROPS,state); return
    fr=Frame(book,cutoff); rid='R%d-%02d'%tuple(cutoff); t0=datetime.datetime.now()
    rb=rollback_check(fr,C,cutoff,rid); rec=dict(id=rid,date=today(),cutoff=cutoff,games=fr.n,champion=C['current'],rollback=rb,applied=[],proposed=[])
    if rb and rb['status']=='rolled back':
        prev=gm.version(C,rb['previous'])
        v=add_version(C,dict(prev['params']),'rollback','Version %d did worse than version %d on the %d games since it went live (%.3f vs %.3f); restored.'%(rb['version'],rb['previous'],rb['games'],rb['new'],rb['old']),cutoff,rid+':rollback')
        rec['applied'].append(dict(category='rollback',version=v)); state.setdefault('changes',{})['rollback']=[dict(seq=seq(cutoff))]
    cands=evaluate(fr,C,cutoff,rid,state=state)
    tested=[c for c in cands if c['status']!='holds']; rec['tested']=len(tested)
    pend=state.get('pending',{}); newpend={}; ready=[]
    for c in tested:
        if c['status']!='passes': continue
        key='%s|%s|%s'%(c['category'],c['size'],''.join('+' if c['to_params'][p]>c['from_params'][p] else '-' for p in c['from_params']))
        old=pend.get(key)
        if old and fr.n-old['games']>=SPEC['confirm_min_new_games']:
            c['status']='confirmed'; ready.append(c); newpend[key]=old
        else:
            c['status']='pending'; newpend[key]=old or dict(review=rid,cutoff=cutoff,games=fr.n)
    ...
    changes=state.setdefault('changes',{}); did_auto=bool(rec['applied']); O=origin_params(C)
    for c in sorted(ready,key=lambda c:-c['net']):
        cat=CATS[c['category']]; last=max([x['seq'] for x in changes.get(c['category'],[])]+[-99])
        season_steps=sum(x['steps'] for x in changes.get(c['category'],[]) if x.get('how')=='auto' and x['season']==cutoff[0])
        drift=steps(cat,vec(cat,O,O),vec(cat,dict(gm.current(C)['params'],**c['to_params']),O))
        c['checks'].append(dict(id='confirmed',ok=True,text='passed in two reviews with new games in between'))
        if c['size']=='small' and season_steps+c['steps']<=1+1e-9 and drift<=2+1e-9:
            ...
            if did_auto:
                c['status']='pending'; c['checks'].append(dict(id='one_at_a_time',ok=False,text='another change applied in this review; re-tested next week against it')); continue
            did=rid+':'+c['category']; P=dict(gm.current(C)['params']); P.update(c['to_params'])
            v=add_version(C,P,'auto',c['summary'],cutoff,did); c['status']='applied'; c['version']=v; did_auto=True
            changes.setdefault(c['category'],[]).append(dict(seq=seq(cutoff),season=cutoff[0],steps=c['steps'],how='auto',version=v))
            rec['applied'].append(dict(category=c['category'],version=v)); jsave(evidence_path(did),dict(decision=did,**c['_evidence']))
        else:
            dec=state.get('declined',{}).get(c['category'])
            if dec and seq(cutoff)<dec: c['status']='declined earlier'; continue
            did=rid+':'+c['category']; c['status']='proposed'; c['proposal']=did
            ...
            PROPS=[x for x in PROPS if not (x['category']==c['category'] and x['status']=='open')]
            PROPS.append(dict(id=did,category=c['category'],name=c['name'],title=c['title'],status='open',created=today(),cutoff=cutoff,reason=reason,
                              from_params=c['from_params'],to_params=c['to_params'],summary=c['summary'],pros=c['pros'],cons=c['cons'],net=c['net'],m=len(tested)))
            rec['proposed'].append(did); jsave(evidence_path(did),dict(decision=did,**c['_evidence']))
    latest=dict(review=rid,cutoff=cutoff,candidates={'%s|%s'%(c['category'],c.get('variant','')):c['_evidence'] for c in tested if c['status'] in ('pending','applied','proposed')})
    jsave(os.path.join(EDIR,'latest.json'),latest)
    rec['results']=[{k:v for k,v in c.items() if not k.startswith('_')} for c in cands]
    rec['seconds']=round((datetime.datetime.now()-t0).total_seconds(),1)
    LOG=[x for x in LOG if x['id']!=rid]+[rec]; LOG=LOG[-26:]
    state['last_cutoff']=cutoff; state['last_games']=fr.n
    jsave(gm.CHAMP,C); jsave(P_LOG,LOG); jsave(P_STATE,state); jsave(P_PROPS,PROPS); write_summary(C,LOG,PROPS,state)
    # one markdown file per new proposal; the workflow opens a GitHub issue for each and never commits them
    os.makedirs(P_NEW,exist_ok=True)
    for f in os.listdir(P_NEW): os.remove(os.path.join(P_NEW,f))
    for x in PROPS:
        if x['id'] in rec['proposed']: open(os.path.join(P_NEW,x['id'].replace(':','_')+'.md'),'w',encoding='utf-8').write(proposal_md(x))
    counts=defaultdict(int)
    for c in cands: counts[c['status']]+=1
    print('review %s: %d games through %d week %d, %d ideas tested in %.0fs -- %s'%(rid,fr.n,cutoff[0],cutoff[1],len(tested),rec['seconds'],dict(counts)))
    if rb: print('  rollback check: %s'%rb)
    for c in cands:
        if c['status']=='holds': continue
        print('  %-11s %-10s %s | net %+.2f'%(c['status'],c['category']+'/'+c['variant'],c['summary'][:150],c['net']))

def proposal_md(x):
    L=['## Model change proposal: %s'%x['name'],'','**%s**'%x['summary'],'','Why it needs your approval: it %s.'%x['reason'],'','**Pros**']
...
def decide(pid,approve):
    C=gm.load_champion(); PROPS=jload(P_PROPS,[]); state=jload(P_STATE,{}); LOG=jload(P_LOG,[])
    x=next((p for p in PROPS if p['id']==pid),None)
    if not x or x['status']!='open': sys.exit('no open proposal %s'%pid)
    book=gm.Book(); cutoff=latest_cutoff(book)
    if not approve:
        x['status']='declined'; x['closed']=today(); state.setdefault('declined',{})[x['category']]=seq(cutoff)+8
        jsave(P_PROPS,PROPS); jsave(P_STATE,state); write_summary(C,LOG,PROPS,state); print('declined %s; %s will not be proposed again for 8 weeks'%(pid,x['name'])); return
    fr=Frame(book,cutoff); rid='A%d-%02d'%tuple(cutoff)
    cs=[c for c in evaluate(fr,C,cutoff,rid,only=x['category']) if c.get('variant')=='full']
    c=cs[0] if cs else None
    if c:   # re-judge with the multiple-testing count of the review that raised it
        c['p_adj']=round(min(1.0,c['p']*x['m']),5); judge(c,x['m'],cutoff,state)
    ok=c is not None and all(k['ok'] for k in c['checks']) and c['net']>0
    if not ok:
        ...
        jsave(P_PROPS,PROPS); write_summary(C,LOG,PROPS,state); print('not applied: %s'%x['why']); return
    P=dict(gm.current(C)['params']); P.update(c['to_params'])
    v=add_version(C,P,'approved','Approved by the owner. '+c['summary'],cutoff,pid)
    x['status']='approved'; x['closed']=today(); x['version']=v; x['applied_params']=c['to_params']
    state.setdefault('changes',{}).setdefault(x['category'],[]).append(dict(seq=seq(cutoff),season=cutoff[0],steps=c['steps'],how='approved',version=v))
    jsave(evidence_path(pid+':approved'),dict(decision=pid,**c['_evidence']))
    jsave(gm.CHAMP,C); jsave(P_PROPS,PROPS); jsave(P_STATE,state); write_summary(C,LOG,PROPS,state)
    print('approved %s -> champion version %d: %s'%(pid,v,c['summary']))

def write_summary(C,LOG,PROPS,state):
    O=origin_params(C); P=gm.current(C)['params']; last=LOG[-1] if LOG else None
    res={r['category']:r for r in (last or {}).get('results',[]) if r.get('variant') in (None,'full')}
    for r in (last or {}).get('results',[]):
        if r['status'] in ('applied','proposed','pending','confirmed'): res[r['category']]=r
    weights=[]
    for cat in SPEC['categories']:
        ch=[v for v in C['versions'] if v['v']>1 and any(v['params'][p]!=gm.version(C,v['v']-1)['params'][p] for p in cat['params'])]
        r=res.get(cat['id'])
        weights.append(dict(id=cat['id'],name=cat['name'],what=cat['what'],value=fmt_ratio(cat,P,O) if cat['kind']=='engine_ratio' else fmt(cat,P),
                            off=is_off(vec(cat,P,O)) if cat['kind'] not in ('engine_ratio','engine','mix') else False,
                            status=(r or {}).get('status','not reviewed yet'),note=(r or {}).get('summary'),
                            last_change=dict(v=ch[-1]['v'],date=ch[-1]['date'],how=ch[-1]['how']) if ch else None))
    notable=[]
    for rev in reversed(LOG):
        for r in rev.get('results',[]):
            if r['status'] in ('applied','proposed','pending','confirmed','watching') or (r['status']=='rejected' and r.get('delta',0)>0):
                notable.append(dict(review=rev['id'],date=rev['date'],**{k:r.get(k) for k in ('category','name','variant','status','title','summary','pros','cons','net','checks','n','base','cand','delta','p_adj','blocks','metric','from_params','to_params','steps')}))
        if len(notable)>=10: break
    S=dict(updated=today(),champion=dict(v=C['current'],date=gm.current(C)['date'],how=gm.current(C)['how'],versions=len(C['versions'])),
           params=P,last_review=None if not last else dict(id=last['id'],date=last['date'],cutoff=last['cutoff'],games=last['games'],tested=last.get('tested',0),
               applied=last['applied'],proposed=last['proposed'],rollback=last.get('rollback')),
           weights=weights,decisions=notable[:10],proposals=[p for p in PROPS if p['status']=='open'],
           history=[dict(v=v['v'],date=v['date'],how=v['how'],note=v['note']) for v in C['versions']],
           rules=dict(alpha=SPEC['alpha'],min_games=SPEC['min_eval_games'],better_blocks=SPEC['min_better_blocks'],cooldown=SPEC['cooldown_weeks'],
                      seasons=SPEC['eval_last_seasons']))
    jsave(P_SUM,S)

if __name__=='__main__':
    ap=argparse.ArgumentParser(); ap.add_argument('cmd',choices=['review','approve','reject','status']); ap.add_argument('id',nargs='?'); ap.add_argument('--force',action='store_true')
    a=ap.parse_args()
    if a.cmd=='review': review(a.force)
    elif a.cmd in ('approve','reject'): decide(a.id,a.cmd=='approve')
    else:
        S=jload(P_SUM,None); print(json.dumps(S and dict(champion=S['champion'],last_review=S['last_review'],proposals=[p['id'] for p in S['proposals']]),indent=1))
