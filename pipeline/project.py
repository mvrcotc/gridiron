"""Player projections in one pass: history -> base model inputs -> weather -> injury redistribution -> simulate.
History is this season's games before the week plus last season's at the learned weight (champion_players.json), so
current-season games feed every projection and rookies are projected once they have played. Adjustments change the
model's INPUTS and every distribution is simulated afterwards, so count stats stay whole numbers, and before/after
comparisons share a per-player seed so noise cancels."""
...
CAL=json.load(open(os.path.join(HERE,'fit_cal.json')))
WXF=json.load(open(os.path.join(HERE,'wxfit.json'))); CEN=json.load(open(os.path.join(HERE,'wxcentre.json')))
...
hits=[(int(BOOK.rows[BOOK.by_espn[g['id']]]['season']),int(BOOK.rows[BOOK.by_espn[g['id']]]['week'])) for g in D['games'] if g['id'] in BOOK.by_espn]
...
base={}; skipped=0; rookies=0
for pid,team in want:
    ctx=GCTX[team]; pos=PANEL.POS.get(pid)
    if ctx['state']=='post' or pid in base: continue
    if pos not in pm.BASE: skipped+=1; continue
    pj=pm.project_one(PANEL,pid,SEASON,WEEK,ctx['spread'],nf(ctx['opp']),pos,P)      # nflverse code: opponent adjustments are keyed LA / WAS
    if not pj or pj['tgt']+pj['car']+pj['pa']<1.5: skipped+=1; continue
    rookies+=int(not any(g['s']==SEASON-1 for g in PANEL.BY.get(pid,[])))
...
for team in sorted({b['team'] for b in base.values()}):
    ...
    ft=sum(base[p]['wx']['tgt'] for p in outs)*P['absorb_tgt']; fc=sum(base[p]['wx']['car'] for p in outs)*P['absorb_car']
...
print('projected %d players for %d week %d (%d ruled out, %d skipped, %d with no games last season); history: this season + last season x%g (player weights version %d); injury fallout on %d teams; fingerprint %s'%(
    len(PROJ),SEASON,WEEK,sum(1 for v in PROJ.values() if v.get('out')),skipped,rookies,P['prev_weight'],PV['v'],len(OUTREC),fp))
